...
The attributes come in free order, with uncataloged abbreviations, in two languages, and
...
| **Material** | Only if a real material appears (`acero`, `STEEL`) | The `MATERIAL` column **almost never** carries it. See P-3 |
| **Quality** | Only if the value is **marked as quality** | Written rule: *"if it isn't known whether a value is marked as quality, it isn't extracted."* ASTM grades (`GR B7`, `GR 2H`) are extracted as-is |
| **Measure** | Value + unit (`7/8"`, `M20`) | Imperial and metric aren't mixed |
| **Length** | Value + unit if present (`130` with no unit → `value: "130", unit: null`) | The unit decision is SPEC-005 via P-4 |
| **Standard** | `DIN…`, `DIN EN…`, `ISO…`, `ASME…`, `ASTM…`, `MSS SP…` | The **element's** standard is extracted, not the row's |
...
| Row 14: `acero` | The only real material in the given MTO. Extracted |
| `8.8` on a nut (rows 11, 13) | Extracted as-is. The incoherence is detected by SPEC-005 |
...
- [x] Row 14's `acero` is extracted as material, and `A2`/`A4` are **not**: they're qualities (§5).
- [x] In row 1 the nut's measure is extracted (`HEX. NUT 7/8"` writes it out) and in row 5 it's
      extrapolated (`2 NUT ASTM A194`, no measure). The distinction is respected.

## Two prompt mistakes that cost a round

1. **`A2`/`A4` as material.** The prompt gave `A4` as an example of material. They're qualities
   (G1/G3), and §5 lists them as such. The same mistake was in the materials table.
2. **A standard put into the quality field.** `gpt-5.4-mini` returned `ASTM F436` as the washer's
   quality in rows 1 and 5 — where the gold says there is no quality and the line goes to review.
   The span verifier **doesn't detect it**, because `ASTM F436` really is in the text: the failure
...
Replaced by the deterministic baseline (`src/pipeline/baseline.ts`): it splits with `findNames` and
attributes by proximity to the preceding name. It doesn't invent. It's the ablation that
quantifies the LLM's real value — the number that answers the criterion "knowing when an agent
isn't needed."

Run with `pnpm run eval -- --ablate=extract`. The harness uses the same normalize → validate →
gold; the only variable is who read the row. The measured figure will be written into
`docs/04-architecture.md` once it's taken; until then the piece **is implemented**, the report is
not.
