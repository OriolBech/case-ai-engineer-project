...
if (!id) {
  console.error('\nUso: pnpm corrections:promote -- <correction-id> [--regression-passed] [--entry-id=<vocab-id>]\n');
  process.exit(1);
}
...
if (!regressionPassed) {
  ...
  console.log(`  pnpm corrections:promote -- ${id} --regression-passed --entry-id=<vocab-id>\n`);
  process.exit(0);
}

try {
  orchestratePromotion(id, { regressionPassed: true, promotedEntryId });
