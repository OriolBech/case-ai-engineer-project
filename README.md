...
```bash
npm run check                  # typecheck + tests  (already working)
npm run rules:audit            # alias provenance + deterministic baseline  (already working)
npm run eval                   # evaluation harness against the gold set  (pending)
npm run eval -- --report       # dumps report to eval/reports/  (pending)
```

Node 26 runs TypeScript natively, so there's no build step for the pipeline or for the harness.
That's why relative imports carry an explicit `.ts` extension (see `tsconfig.json`).

## What it does

An MTO Excel goes in. Each row can describe a single material or a complete **set** (bolt +
its nuts + its washers), in which case it's split into one line per element. For each output
line, seven attributes are extracted and normalized, and the line ends up in one of two states:
...
- `REVISION_MANUAL` (MANUAL_REVIEW) — a mandatory attribute is missing or there's an
  inconsistency, with a typed reason.
...
| `data/gold/` | Hand-labeled gold set. The reference against which measurements are taken. |
...
- **No build step**: Node runs TypeScript in *strip-only* mode. That implies two
  constraints across all the code: relative imports with an explicit `.ts` extension, and **no
